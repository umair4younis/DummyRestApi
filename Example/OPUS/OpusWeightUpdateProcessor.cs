...
{{
    assets(
        ...
        {
            const string noResultAssetsQuery = @"{
    assets(filter: { and: [{ expression: ""id = -1"" }] }) {
        edges {
            node {
                id
            }
    ...
    edges {
      node {
        ...
        symbols { identifier }
            ...
            Engine.Instance.Log.Debug("[BuildBbgFilterQuery] GraphQL query constructed successfully. QueryLength=" + query.Length + ", TickerCount=" + sanitizedTickers.Count);
